...
class CompanyController extends Controller_Component {
  ...
  public function init() {
   ...
   /* history grid*/
      //$companyHistoryGrid = $this->addGrid(new InspectsGrid('company_history', null, null, $inspectionId));\

   // $companyHistoryGrid = $this->addGrid(new InspectsGrid('company_history', $companyInspectionsGrid, 'INSPECTION_ID',$inspectionId));

   $companyHistoryGrid = $this->addGrid(new InspectsGrid('company_history', null, null, $inspectionId));



       // $access['company_history'] = $this->getAccessControl()->getGridPermissions('company_history');

      // $this->addGrid(new InspectsGrid('company_history', $companyInspectionsGrid, 'INSPECTION_ID'));
...
public function buildingsAction() {
  ...
  $this->view->readonly = ($acl->inheritsRole($user, 'Building Owners', true)) ? true : false;
...
public function changeAction(){
 $this->_detailedView('company_history');
 $this->view->grid = $this->_components['company_history'];
}
...
public function doorAction() {
  ...
  $this->_detailedView('company_doors'); 
        ...
        if ($doorId < 0) { // new door
          ...
          $building = Model_Door::retrieve()->fetchEntry(null,array('BUILDING_ID', new Data_Column('ID', $doorId)));
          $building = Model_Building::retrieve()->fetchEntry($building['BUILDING_ID']);
          $this->view->buildingName = $building['NAME'];
...
public function dropdownAction() {
  ...
  if ($this->getParam('_model') == 'company' && $this->getParam('_column') == 'PRIMARY_CONTACT') {
    $column = $this->_components['company']->getColumnById('PRIMARY_CONTACT');
