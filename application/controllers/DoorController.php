...
class DoorController extends Controller_Component {
	...
	public function savedoorAction(){
		...
		$fieldIds = array(
			...
			'FIRE_RATING_4','FIRE_RATING_5', 'TEMP_RISE', 'LISTING_AGENCY', 'LISTING_AGENCY_OTHER', 'BARCODE', 'GAUGE', 'MANUFACTURER', 'MODEL', 'REMARKS',
			...
			'FRAME_BOTTOM_TO_CENTER','FRAME_BOTTOM_TO_CENTER_FRACTION', 'STRIKE_HEIGHT',
		...
		foreach($fieldIds as $fieldId){
			switch($fieldId){
				...
				case 'FRAME_BOTTOM_TO_CENTER_FRACTION':
				case 'STRIKE_HEIGHT':
		...
		//Now, let's update the door Recomendataion
		Model_Door::retrieve()->cleanRecomendation($doorId);
		$recomendations = $data['DOOR_RECOMEND_TEXT'];
		Model_Door::retrieve()->setcleanRecomendation($doorId, $recomendations);

		//Now, let's update the door Recomendataion
		Model_Door::retrieve()->cleanHardware($doorId);
		$hardware = $data['HARDWARE_CHECKLIST'];
		Model_Door::retrieve()->setcleanHardware($doorId, $hardware);
	...
	public function exceldoorAction() {
		...
		$str .= "</table>";
		$this->view->placeholder('data')->set($str);
	}
