...
require_once APPLICATION_PATH . '/models/Recomendation.php';
...
class Model_Door extends Model_Abstract{
	...
	protected function _init(){
		...
		$this->addReferenceModel('FIRE_RATING_5', Model_Dictionary::retrieve());
		...
		$this->addReferenceModel('FRAME_BOTTOM_TO_CENTER_FRACTION', Model_Dictionary::retrieve()); 
	...
	public function save($data, $ignored=null, $clean=true){
		...
		$data['UPDATED_AT'] = $now = date ('Y-m-d H:i:s', time());

		/*set if radio button is empty */
		if(isset($data['STYLE']) && $data['STYLE'] == "") $data['STYLE'] = null;
		if(isset($data['MATERIAL']) && $data['MATERIAL'] == "") $data['MATERIAL'] = null;
		if(isset($data['ELEVATION']) && $data['ELEVATION'] == "") $data['ELEVATION'] = null;
		if(isset($data['FRAME_MATERIAL']) && $data['FRAME_MATERIAL'] == "") $data['FRAME_MATERIAL'] = null;
		if(isset($data['FRAME_ELEVATION']) && $data['FRAME_ELEVATION'] == "") $data['FRAME_ELEVATION'] = null;
		if(isset($data['FIRE_RATING_1']) && $data['FIRE_RATING_1'] == "") $data['FIRE_RATING_1'] = null;
		if(isset($data['LISTING_AGENCY']) && $data['LISTING_AGENCY'] == "") $data['LISTING_AGENCY'] = null;
		if(isset($data['HINGE_HEIGHT']) && $data['HINGE_HEIGHT'] == "") $data['HINGE_HEIGHT'] = null;
	...
	/*
	 * The function deletes all the Recomendation for a specific door 
	 */
	public function cleanRecomendation($doorId){
		if (!$doorId) return;
		Model_Recomendation::retrieve()->deleteEntries(array(new Data_Column('DOOR_ID', $doorId)));
	}
	
	/**
	 * The function sets the door Recomendation for a specific door
	 */
	public function setcleanRecomendation($doorId, $text){
		if (!$doorId) return;
		Model_Recomendation::retrieve()->save(array(
			'DOOR_ID' => $doorId,
			'RECOMENDATION' => $text
		));
	...
	/*
	 * The function deletes all the Hardware for a specific door 
	 */
	public function cleanHardware($doorId){
		if (!$doorId) return;
		Model_Hardware::retrieve()->deleteEntries(array(new Data_Column('DOOR_ID', $doorId)));
	}

	/**
	 * The function sets the door Recomendation for a specific door
	 */
	public function setcleanHardware($doorId, $codes){
		if (!$doorId) return;
	...
	*/
	public function setDoorTypes($doorId, $types){
		...
		$data = Model_Dictionary::retrieve()->fetchEntries(array(
			'ID', 'ITEM', 'VALUE_ORDER', new Data_Column('CATEGORY', 'Door Type')), null, true, 'VALUE_ORDER'); 
		
		$temp = [];
		foreach ($data as $value) {
			if(in_array($value['ID'],$types) ){
	...
	public function savePicture($doorId, $picture, $pictureId, $pictureNote, $pictureOrder) {
		...
		$result[$pictureId] = '/content/pictures/' . $targetFileName;
		return $result;
	}
	...
	public function saveAudio($doorId, $audio, $audioId, $audioNote) {
		...
		$result[$audioId] = '/content/audio/' . $targetFileName;
