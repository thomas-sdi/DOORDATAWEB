...
class EmployeeGrid extends Cmp_Custom_Grid {
	...
	public function __construct($id, $parent=null, $parentLink=null, $inspectionVSowner='inspection', $companyId) {
        ...
        $columns = array(
        	...
        	'first_name'       => array('Title'    => 'First Name','Maxlength' => '12', 'Id' => 'FIRST_NAME'),
        	'last_name'        => array('Title'    => 'Last Name', 'Maxlength' => '30','Id' => 'LAST_NAME'),
		...
		if ($inspectionVSowner == 'inspection'){
			...
			$columns['license_number'] = array('Title'   => 'License Number', 'Maxlength' => '8', 'Visible' => '011'); 
    ...
    public function calculateEmployeeActions($entry){
       ...
       <i class="glyph-icon icon-chevron-down"></i>
		...
		$editAction 	= '<div class="pad5A button-pane button-pane-alt text-center"><a data-original-title="Edit" title="Edit" data-placement="top" class="btn btn-sm hover-blue-alt tooltip-button" 
						href="javascript: cmp_' . $gridId . '.showEditDialog('. $entry['ID'] .')"><i class="fa fa-edit"></i>Edit</a></div>';
		$deleteAction = '<div class="pad5A button-pane button-pane-alt text-center"><a data-original-title="Delete" title="Delete" data-placement="top" class="btn btn-sm hover-red tooltip-button" 
		...
		if ($acl->inheritsRole($user, 'Building Owner Employees', true)) {
			...
			if ($user == $login) {
				...
				if ($email) {
					$emailAction = '<div class="pad5A button-pane button-pane-alt text-center"><a data-original-title="Email" title="Email" data-placement="top" class="btn btn-sm hover-blue-alt tooltip-button" 
						href="mailto:'. $email .'"><i class="glyph-icon icon-envelope-o"></i>Email</a></div>';
