	...
	public function __construct($id, $parent = null, $parentLink = null, $inspectionId = -1) {
		...
		$columns = array(
			...
			'TYPE_OTHER' => array(
				'Id'		=> 'TYPE_OTHER',
				'Hidden' 	=> true,
				'View' 		=> Data_Column_Grid::TEXTAREA,
				'Visible' 	=> '000', 
				'Maxlength' => '25'),
			'MATERIAL' => array(
				'Id'				=> 'MATERIAL',
			...
			'FIRE_RATING_5' => array(
				...
				'Visible' 			=> '000', // 21
				'View' 				=> Data_Column_Grid::RADIO,
				'DropdownFilter' 	=> new Data_Column('category', 'Fire-Rating 5', Model_Dictionary::retrieve()),
				'Title' 			=> ''),
			'TEMP_RISE' => array(
				'Id'				=> 'TEMP_RISE',
				'Hidden' 			=> true, // 22
			...
			'FRAME_BOTTOM_TO_CENTER_FRACTION' => array(
				'Id'				=> 'FRAME_BOTTOM_TO_CENTER_FRACTION',
				'Hidden' 			=> true,
				'DropdownFilter' 	=> new Data_Column('category', 'Frame Bottom To Center Fraction', Model_Dictionary::retrieve()),
				'Visible' 			=> '000'
			),
			'STRIKE_HEIGHT' => array(
				'Id'				=> 'STRIKE_HEIGHT',
...
public function calculateCompliant($entry) {
	...
	if ($comp) {
		if ($comp == 'No') {
			$comp = "<span style='color:#ae1916;text-align: left;display:block; padding-left:15px;'> " . $comp ."</span>";
		}
		elseif ($comp == 'Yes') {
			$comp = "<span style='color:#578726;text-align: right;display:block;padding-right:15px;'> " . $comp ."</span>";
		}
	} 
...
public function calculateFireRating($entry) {
	...
	$fr5 = $entry['FIRE_RATING_5']  ? $entry['FIRE_RATING_5'] . ', ' : '';

		$field = $fr1 . $fr2; //remove the last two characters
		...
		$field .= substr($fr3 . $fr4 . $fr5, 0, -2); //remove the last two characters
	...
	public function calculateDoorActions($entry) {
		...
		href="javascript: cmp_' . $gridId . '.showDoorEditDialog('. $entry['ID'] .', \'ordinary\');"><i class="fa fa-edit"></i>Edit</a></div>';
		...
		href="#"><i class="glyph-icon icon-edit"></i>Edit</a></div>';	
		
		//$inspect = "<a href='javascript: gridDialogDoor.showEdit(\"checklist\")'>Inspect</a>";
		$inspect = '<div class="pad5A button-pane button-pane-alt text-center"><a data-original-title="Inspect" title="Inspect" data-placement="top" class="btn btn-sm hover-blue-alt tooltip-button" 
		href="javascript: cmp_' . $gridId . '.showDoorEditDialog('. $entry['ID'] .', \'checklist\');"><i class="glyph-icon icon-check"></i>Inspect</a></div>';
		
		
		$inspectDisabled = '<div class="pad5A button-pane button-pane-alt text-center"><a data-original-title="Inspect" title="Inspect" data-placement="top" class="btn btn-sm tooltip-button" style="color: gray;"
		href="#"><i class="glyph-icon icon-check"></i>inspect</a></div>';	
		...
		href="javascript: cmp_' . $gridId . '.showDoorEditDialog('. $entry['ID'] .', \'ordinary\');"><i class="glyph-icon icon-eye"></i>View</a></div>';
		
		$actions = '';
		if ($inspectionStatus == Model_Inspection::PENDING || $inspectionStatus == Model_Inspection::INCOMPLETED){
			$actions = $actionHead . $edit . $inspect. $delete . $actionFoot;
		}
		if ($inspectionStatus == Model_Inspection::SUBMITTING || $inspectionStatus == Model_Inspection::SUBMITTED || $inspectionStatus == Model_Inspection::COMPLETED){
			$actions = $actionHead . $view . $actionFoot;
		}
		return $actions;
	}
...
public function fetchExcel($queryParams=null, $start=0, $sortBy=null, $sortDirection=true ) {
	...
	foreach ($entries as $entry) {
		$entry['_parent']= $queryParams['_super'];

			// retrieve the custom codes
		$otherCodes = Model_Door::retrieve()->getOthers(null, $entry['_parent']);

			// pull all the door codes within this inspection
		$allDoorCodes = array();
		foreach (Model_DoorCode::retrieve()->fetchEntries(array(
			'DOOR_ID',
			new Data_Column('DOOR_ID', $entry['_parent'], Model_DoorCode::retrieve(), 'INSPECTION_ID'),
		...
		foreach ($entry as $columnId => $value) {
			...
			if ($columnId == '_parent' || $columnId == '_parent')
				continue;
