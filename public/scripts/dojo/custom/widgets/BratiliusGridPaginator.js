dojo.provide("custom.widgets.BratiliusGridPaginator");
dojo.provide("custom.widgets.BratiliusGridPaginatorPage");
dojo.provide("custom.widgets.BratiliusGridPaginatorList");


dojo.declare("custom.widgets.BratiliusGridPaginator", [dijit._Widget, dijit._Templated, dijit._Container], {
	templatePath: dojo.moduleUrl("custom", "templates/BratiliusGridPaginator.html"),
	grid		: "",
	pageCount	: 0,
	curPage		: null,
    curPageIndex: 0,
    ...
    startup: function(){
    	...
    	console.log("paginate value", this.rowsPerPage);
	...
	selectPage: function(/*Integer*/pageIndex,  /*Boolean*/refresh) {
		...
		if (refresh) this.grid.goToPage(pageIndex,this.rowsPerPage);
	...
	firstClick: function(evt) {
		this.selectPage(0, true);
	},

	lastClick: function(evt) {
		this.selectPage(this.pageCount - 1, true);
	}

});


dojo.declare("custom.widgets.BratiliusGridPaginatorPage", [dijit._Widget, dijit._Templated, dijit._Contained], {
	templatePath: dojo.moduleUrl("custom", "templates/BratiliusGridPaginatorPage.html"),
	text	: 'null',
	index	: 0,
...
dojo.declare("custom.widgets.BratiliusGridPaginatorList", [dijit._Widget, dijit._Templated, dijit._Container], {
    ...
    prevNode	: null,
    nextNode	: null,
    rowsPerPage	: this.rowsPerPage ? this.rowsPerPage :20,
    ...
    startup: function() {
		...
		this.refreshPaginatorStyles(this.curPageIndex);
	},

	
	refreshPaginatorStyles: function(pageIndex){
		if (pageIndex < 0 || pageIndex >= this.pageCount || isNaN(pageIndex))
			return;
	...
	//Load page if refresh=true
	selectPage: function(/*Integer*/pageIndex, /*Boolean*/refresh,/*Integer*/rowsPerPage=null,) {
		
		// if (pageIndex < 0 || pageIndex >= this.pageCount || isNaN(pageIndex))
		// 	return;
		this.refreshPaginatorStyles(pageIndex);
		console.log("event",pageIndex,rowsPerPage);

		if (refresh) this.grid.goToPage(pageIndex,rowsPerPage);
			
	},
	
	click10: function(evt) {
		this.selectPage(this.curPageIndex,  true,10);
	},
	
	click20: function(evt) {
		this.selectPage(this.curPageIndex, true, 20);
	},
	click50: function(evt) {
		this.selectPage(this.curPageIndex, true, 50);
	},
	
	click100: function(evt) {
		this.selectPage(this.curPageIndex,true, 100);
	}
});