...
dojo.declare("custom.widgets.BratiliusGrid", ginger.Widget, {
	...
	goToPage: function(pageIndex, rowsPerPage=20) {
		if (!this.parentPane) {
		...
		if (qPos < 0)
			href += '?page=' + pageIndex + '&rowsPerPage=' + rowsPerPage;
		else {
			...
			if (pagePos < 0)
				...
				endPos = href.indexOf(';');
				if (endPos < 0)
					href = href.substr(0,pagePos) + 'page=' + pageIndex + '&rowsPerPage=' + rowsPerPage;
				else
					href = href.substr(0,pagePos) + 'page=' + pageIndex + href.substr(endPos) + '&rowsPerPage=' + rowsPerPage;
			}
		}
